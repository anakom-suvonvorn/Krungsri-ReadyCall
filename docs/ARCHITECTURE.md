...
| **Comparison** (`services/comparison/`) | Compare & best-fit (`D126`): ranking candidate plans on real figures against what the customer holds, and composing the table the customer's screen renders. The catalogue arrives through `CoreDataProvider` (`D125`); the weights come from `config/comparison.yaml`. **No I/O of its own, deliberately** — the model that writes the reason sentence lives outside it in `services/analysis/comparison_reason.py` (`D137`), so the ranking stays testable against a list of floats with no key and no network. | Rank with a model, quote a premium, or let a client supply the figures. |
