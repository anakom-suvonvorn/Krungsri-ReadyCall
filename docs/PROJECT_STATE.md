...
| **THE ENGINE** (`D30`, `D103`) | **CT2 `int8_float16` WITH the vocabulary hint**, converted once by `scripts/convert_ct2.py`. `STT_ENGINE=thonburian_ct2`, `STT_MODEL` left unset (`B23`). Chosen as a deliberate trade: **17% relatively more character error for 12-24x on latency and 2.7x less VRAM** |
