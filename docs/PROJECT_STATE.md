...
Verified on 2026-09-02: **666 tests** — 605 pass + 42 skipped without the Postgres container
...
| **THE ENGINE** (`D30`, `D102`) | **CT2 `int8_float16`**, converted once by `scripts/convert_ct2.py`. Chosen on measurement, paced, over 12 real calls: p95 **1.65 s median / 2.48 s worst** against Thonburian fp16's **19.5 s / 58.7 s**; `busy` worst **0.13** against **1.48**; VRAM **1106 MB** against **2731 MB**. Costs CER median 0.161 -> **0.182**, which is a real but small price for 12-24x on latency |
| **Real Thai accuracy** (`D97`, `B21`, `D102`) | Thonburian medium + Silero over 12 real call-centre calls: **fp16 CER median 0.161**, **CT2 int8 0.182**. Was 0.292 until `B21` stopped the guard deleting phone numbers (8 of 12 improved, none worse). The earlier **0.47-0.76** was measured through `B20` and is withdrawn. Residual is partly scoring, not error (`Q28`, the `CERth` column). **Measured on a digit-heavy set** (`Q30`) - re-run on a mixed one before quoting as the engine's CER |
| **Latency against the budget** (`ARCHITECTURE` §15) | Budget p95 **< 1.5 s**. CT2 achieves **1.65 s median, 2.48 s worst, 3 of 12 calls inside**. Missed by 1.1-1.7x rather than the 13-39x of fp16. Remaining levers: the cached **distil** checkpoint, packing (`D101`), Typhoon (`D99`) |
| **Throughput** | `busy` = model-seconds per second of audio, and above 1.00 the transcriber never catches up. fp16 **0.45 median / 1.48 worst, 1 call over 1.00**; CT2 **0.09 / 0.13, none over**. `pad` = seconds Whisper encoded per second of call: **2.2-3.0, median 2.7**, because it pads every clip to a fixed 30 s window - that multiple is the prize for packing |
