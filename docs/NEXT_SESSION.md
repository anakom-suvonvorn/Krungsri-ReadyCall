...
| `D132` | a **configurable test call** | two buttons: the plain one unchanged, and one that opens a dialog reporting what the machine is actually configured as. Assurance and urgency are **shown, not chosen** — the ladder is derived from evidence (`D20`), so a picker offering L3 would assert a level with nothing behind it. Three guards, each of which caught a real trap while verifying |
| `D133` | the customer can **type** their intake | ⚠️ **the whole AI story now runs with no audio at all** — typed text is a turn that never needed a model (`D88`), so the summary lands on the offer card with no microphone, no WAV and no GPU. The app asks before dialling, because `INTAKE_ACTIVE` is reachable only from `QUEUED` and there is no window during the wait (`Q39`) |
| `B40` | a pushed comparison table **stretched the app** | and every card after it. `min-width: auto` up the whole grid chain, so `overflow-x` had nothing to scroll inside — and it only reproduced below 860 px, where a bare `1fr` means `minmax(**auto**, 1fr)` |
